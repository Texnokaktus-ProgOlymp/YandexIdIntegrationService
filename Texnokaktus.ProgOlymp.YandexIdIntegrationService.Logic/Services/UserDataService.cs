...
internal class UserDataService(IYandexAuthenticationService authenticationService,
                               ...
                               IUnitOfWork unitOfWork) : IUserDataService
{
    ...
    {
        var token = await authenticationService.GetAccessTokenAsync(code);
        var userData = await yandexIdClient.GetUserDataAsync(token.AccessToken);
...
{
    public static User MapUser(this YandexClient.Models.UserData userData) =>
        new(userData.Login,
